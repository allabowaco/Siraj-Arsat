...
window.showView = async (view, btn) => {
    ...
    // ==========================================
    const viewButtons =
        document.querySelectorAll('[id^="btn-view-"]');
    ...
    if (btn) {
        ...
        const targetBtn =
            document.getElementById(`btn-view-${view}`);

        if (targetBtn) {
            targetBtn.classList.add('active-view');
        }
    }
    ...
    // ==========================================
    const systemButtons =
        document.querySelectorAll('[id^="btn-sys-"]');

    // إزالة التحديد من الكل أولاً
    systemButtons.forEach(btn => {
        btn.classList.remove('active-system');
    });

    // ==========================================
    // إظهار/إخفاء زر الرئيسية والتقارير
    // ==========================================
    const homeBtn =
        document.getElementById('btn-view-home');

    const reportsBtn =
        document.getElementById('btn-view-reports');

    // أزرار FAB
    const fabSwitch =
        document.getElementById('fabSwitch');

    const fabRefresh =
        document.getElementById('fabRefresh');
    ...
    const shouldHideNavButtons =
        hideNavButtonsViews.includes(view);
    ...
    // ==========================================
    // تقرير العمولات -> تحويل للحركات المالية
    // ==========================================
    if (view === 'commissionReports') {
    ...
    if (
        ...
        const activeSystemBtn =
            document.getElementById(
                `btn-sys-${currentSystem}`
            );

        if (activeSystemBtn) {
            activeSystemBtn.classList.add('active-system');
        }
    }
    ...
    // صفحة تقرير العمولات
    // ==========================================
    else if (view === 'commissionReports') {
...
document.getElementById('accountForm').onsubmit = async (e) => {
    ...
    const id =
        ...
        const data = {
            name: document.getElementById('companyAccountName').value,

            withdrawCommission: parseNumber(
                document.getElementById('withdrawCommission').value
            ),

            depositCommission: parseNumber(
                document.getElementById('depositCommission').value
            ),

            createdAt: new Date(),

            sortOrder: companyAccounts.length + 1
        };
    ...
    try {
        ...
        if (id) {
            ...
            await addDoc(
                collection(db, 'company_accounts'),
                data
            );
        ...
        document.getElementById('saveAccountBtn').innerHTML =
            'حفظ الحساب';

        document.getElementById('saveAccountBtn').className =
            'btn btn-warning w-100 fw-bold py-3';
    } catch {

        showToast('فشل حفظ الحساب ❌', 'danger');
    ...
    companyAccounts.forEach(acc => {
        ...
        body.innerHTML += `
            ...
            <tr data-id="${acc.id}">
                ...
                <td>
                    ${formatNumber(acc.withdrawCommission)} د.ع
                </td>

                <td>
                    ${formatNumber(acc.depositCommission)} د.ع
                </td>

                <td>

                    <button class="btn btn-info btn-sm"
                            onclick="editCompanyAccount('${acc.id}')">
                    ...
                    <button class="btn btn-danger btn-sm"
                            onclick="deleteCompanyAccount('${acc.id}')">
...
function initAccountsSortable() {
    ...
    tbody.sortableInstance = new Sortable(
        tbody,
        {
            animation: 150,
...
window.editCompanyAccount = (id) => {

    const acc =
        companyAccounts.find(x => x.id === id);
    ...
    document.getElementById('companyAccountName').value =
        acc.name;

    document.getElementById('withdrawCommission').value =
        formatNumber(acc.withdrawCommission);

    document.getElementById('depositCommission').value =
        formatNumber(acc.depositCommission);

    document.getElementById('saveAccountBtn').innerHTML =
        '<i class="bi bi-pencil-fill me-2"></i> تحديث الحساب';

    document.getElementById('saveAccountBtn').className =
        'btn btn-info w-100 fw-bold py-3 text-white';

};

window.deleteCompanyAccount = (id) => {
    ...
    document.getElementById('modalText').innerText =
        `هل أنت متأكد من حذف حساب (${acc.name}) ؟`;

    document.getElementById('confirmActionBtn').className =
        'btn btn-danger btn-modern px-5';
...
function updateCommissionCards(companyTotal, userTotal) {

    const container =
        document.getElementById('commissionKpis');

    if (!container) return;

    const totalCommission =
        Math.round(companyTotal) +
        Math.round(userTotal);
    ...
    container.innerHTML = `
        ...
        <div class="row g-3 mb-4">
                ...
                <div class="card border-0 shadow-sm bg-warning bg-opacity-10 border-start border-warning border-5">
                    <div class="card-body">
                        <small class="text-warning fw-bold">
                            مجموع العمولات الكلية
                        </small>

                        <h3 class="m-0 fw-bold text-warning">
                            ${formatNumber(totalCommission)} د.ع
                        </h3>
                    </div>
                </div>
            </div>
            ...
            <!-- عمولات الشركة -->
            <div class="col-md-4">
                <div class="card border-0 shadow-sm bg-success bg-opacity-10 border-start border-success border-5">
                    <div class="card-body">
                        <small class="text-success fw-bold">
                            مجموع عمولات الشركة الكلي
                        </small>

                        <h3 class="m-0 fw-bold text-success">
                            ${formatNumber(Math.round(companyTotal))} د.ع
                        </h3>
                    </div>
                </div>
            </div>
                ...
                <div class="card border-0 shadow-sm bg-primary bg-opacity-10 border-start border-primary border-5">
                    <div class="card-body">
                        <small class="text-primary fw-bold">
                            مجموع عمولات الزبائن الكلي
                        </small>

                        <h3 class="m-0 fw-bold text-primary">
                            ${formatNumber(Math.round(userTotal))} د.ع
                        </h3>
                    </div>
                </div>
            </div>
...
// 2. دالة رندر تقرير العمولات المحدثة
async function renderCommissionReport() {

    const body =
        document.getElementById('commissionReportBody');
    ...
    try {
        ...
        const dataSource = snapshot.docs.map(doc => ({
            id: doc.id,
            ...doc.data()
        }));
        ...
        dataSource.forEach(item => {
            ...
            if (accountData && amount > 0) {
                ...
                if (
                    ...
                    companyRate =
                        parseNumber(
                            accountData.withdrawCommission || 0
                        );
                ...
                // قبض
                    ...
                    companyRate =
                        parseNumber(
                            accountData.depositCommission || 0
                        );
                }

                companyCommission =
                    companyRate * millionsFactor;
            }
            ...
            groups[key].userTotal +=
                userCommission;

            groups[key].companyTotal +=
                companyCommission;
        });
...
// دالة مساعدة لعرض جدول فارغ
function showEmptyCommissionTable() {
    const body = document.getElementById('commissionReportBody');
    if (body) {
        body.innerHTML = `
            <tr>
                <td colspan="5" class="text-center text-muted py-4">
                    لا توجد عمولات مسجلة في الحركات الحالية
                </td>
            </tr>
        `;
    }
...
function loadCompanyAccounts() {
    ...
    onSnapshot(q, (snapshot) => {
        ...
        companyAccounts = snapshot.docs.map(doc => ({
            id: doc.id,
            ...doc.data()
        }));
...
document.getElementById('mainForm').onsubmit = async (e) => {
    ...
    // منع حفظ التصريف بدون سعر
    if (currentSystem === 'exchange') {

        const rateValue = parseNumber(
        ...
        if (rateValue <= 0) {
            ...
            showToast(
                'يجب إدخال سعر تصريف أكبر من صفر ❌',
                'danger'
            );
...
function showToast(msg, type = 'success') {
    ...
    setTimeout(() => document.getElementById(`toast-${id}`)?.remove(), 2000);
}
...
function updateFabLabels() {
    ...
    if (currentSystem === 'exchange') {
        fabSwitch.querySelector('i').className = 'bi bi-cash-coin';
        fabSwitch.querySelector('span').textContent = 'الحركات المالية';
    } else {
        fabSwitch.querySelector('i').className = 'bi bi-currency-dollar';
...
function setupDailyDifferenceCalculator() {

    const input =
        document.getElementById('dailyRateInput');

    const result =
        document.getElementById('dailyDifferenceResult');
    ...
    input.addEventListener('input', () => {
        ...
        if (!dailyRate || dailyRate <= 0) {
            ...
            result.className =
                'alert alert-secondary text-center fw-bold fs-5 mb-0';

            result.innerHTML =
                'ادخل سعر التصريف لعرض النتيجة';
        ...
        if (totalSell > totalBuy) {
            ...
            const iraqiAmount =
                difference * dailyRate;

            result.className =
                'alert alert-success text-center fw-bold fs-5 mb-0';

            result.innerHTML = `

                <div class="mb-2">

                    البيع الكلي - الشراء الكلي
        ...
        // ==========================================
            ...
            result.className =
                'alert alert-danger text-center fw-bold fs-5 mb-0';

                    result.innerHTML = `

                        <div class="mb-2">

                            الشراء الكلي - البيع الكلي
        ...
        // ==========================================
            ...
            result.className =
                'alert alert-info text-center fw-bold fs-5 mb-0';

            result.innerHTML =
                'البيع والشراء متساويان';
        }
...
window.renderMatchView = async () => {
    ...
    try {
        ...
        setupDailyDifferenceCalculator();
...
window.executeQuickMatch = async () => {

    const actualBalance = parseNumber(
        document.getElementById(
            'actualBalanceInput'
        ).value
    );
    ...
    if (
        ...
        showToast(
            'يجب إدخال رصيد صحيح أكبر أو يساوي صفر',
            'warning'
        );
    ...
    try {
        ...
        // الرصيد الجديد = الرصيد الفعلي المدخل
        const newBalance =
            actualBalance;
        ...
        showToast(
            'تمت المطابقة السريعة وتصفير السجل بنجاح ✅',
            'success'
        );
...
window.updateQuickMatchPreview = () => {
...
async function initPushNotifications(retryCount = 0) {
    ...
    try {
        ...
        const token = await getToken(messaging, {
            vapidKey:
                "BCTjas5eRGNg2kpYtQ_Bc3yNT4q9QVyaDInrhmJREIICIBE1eGHuWO6VPKv03-2e4cGNHW0lxybgE3H7BmlMy-A",
...
window.increaseTableFont = () => {

    let size =
        parseInt(
            localStorage.getItem('table-font-size')
            || 16
        );
...
window.openQuickMatchModal = async () => {
    ...
    // تنظيف أي نتائج قديمة
    document.getElementById(
        'quickMatchResult'
    ).innerHTML = '';

    // وضع الرصيد المتوقع داخل الحقل
    document.getElementById(
        'actualBalanceInput'
...
function checkForOldRecords() {
    ...
    hasOldRecords = currentData.some(item => {
        ...
        if (item.timestamp?.toDate) {
            ...
            recordDate = new Date(item.timestamp);
        }

        const recordString =
            `${recordDate.getFullYear()}-${recordDate.getMonth()}-${recordDate.getDate()}`;
